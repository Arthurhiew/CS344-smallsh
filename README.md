...
Submission
 Submitted!
Mar 3 at 7:26pm (late)
Submission Details
Download smallsh-2.zip
Grade: 169 (180 pts possible)
Graded Anonymously: no
Comments:
Submission is missing a readme. Student has been notified to resubmit; deadline for resubmission is March 5, 11:59 PM.
Michael Andrews, Mar 3 at 6:37pm
-8 Resubmission The script looks good (although there's some weirdness in an echo which contains an &). I am going to dock you three points for signal handling, though. For one thing, if you catch SIGTSTP while a foreground process is running, the spec says you should delay printing until after the process finishes. You print immediately, but worse than that, you let that interrupt the wait() for the FG process: the result is that you get an immediate new prompt, and the FG process ends up as a permanent zombie. Another issue is that you're supposed to allow foreground processes to catch SIGINT, but prevent background processes from doing so. Instead, the only circumstance in which you allow any child to catch SIGINT is when it's a foreground process _and_ there's redirection. And when you do so, you do it by registering a custom handler, but custom handlers aren't inherited through exec: they're replaced with SIG_DFL, so your handler doesn't actually do anything at all. (Which is probably good, since in this case your handler doesn't actually call exit()...) As a result, you only allow foreground processes to be terminated by Ctrl-C in very specific circumstancs, and even then you don't print the required message.
hiew-smallsh1.png
Michael Andrews, Mar 4 at 12:59pm
­
